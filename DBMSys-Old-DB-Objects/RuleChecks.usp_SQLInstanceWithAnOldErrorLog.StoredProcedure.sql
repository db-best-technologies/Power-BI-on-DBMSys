...
GO
/****** Object:  StoredProcedure [RuleChecks].[usp_SQLInstanceWithAnOldErrorLog]    Script Date: 6/8/2020 1:14:33 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create procedure [RuleChecks].[usp_SQLInstanceWithAnOldErrorLog]
	@ClientID int,
	@PRR_ID int,
	@FromDate date,
	@ToDate date,
...
select @ClientID, @PRR_ID, MOB_ID, datediff(day, DID_CurrentErrorLogStartDate, @LastDate)
...
where (@UpperValue is not null
		and DID_CurrentErrorLogStartDate >= dateadd(day, -@UpperValue, @LastDate))
	and (@LowerValue is not null
		and DID_CurrentErrorLogStartDate <= dateadd(day, -@LowerValue, @LastDate))
GO
