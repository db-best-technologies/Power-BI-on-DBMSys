...
GO
/****** Object:  StoredProcedure [Activity].[AGReplicaEstimatedDataloss]    Script Date: 6/8/2020 1:14:32 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [Activity].[AGReplicaEstimatedDataloss]
--DECLARE
...
select	h_all.AGH_MOB_ID F_MOB_ID
		...
		,'Replica''s ' + h_all.AGH_ReplicaName + '\' + h_all.AGH_DatabaseName + ' estimated Data loss. '
		+ ISNULL(CHAR(10) + 'Replica name'					+': ' + m.MOB_Name						,'')
...
CROSS APPLY (
				...
				WHERE	h_par.AGH_GroupID				= h_all.AGH_GroupID	
						...
						AND	ISNULL(h_par.AGH_LastLSN,0)	> ISNULL(h_all.AGH_LastLSN,0)

			)relp_par
where	MOB_OOS_ID = 1 
		AND AGH_SyncState = 2 
		AND DATEDIFF(HH,'19000101',h_all.AGH_LastHardenedLSN) > 2--DATEPART(hh,h_all.AGH_LastHardenedLSN)>2
GO
