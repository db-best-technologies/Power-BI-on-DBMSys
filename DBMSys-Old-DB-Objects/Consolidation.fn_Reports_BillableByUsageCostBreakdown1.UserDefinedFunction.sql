...
GO
/****** Object:  UserDefinedFunction [Consolidation].[fn_Reports_BillableByUsageCostBreakdown1]    Script Date: 6/8/2020 1:14:32 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE function [Consolidation].[fn_Reports_BillableByUsageCostBreakdown1] (@CLV_ID tinyint) returns table
as
return (with Pricing as
    (select BUL_CLV_ID CLV_ID, ItemType, ItemLevel, FromValue, ToValue, Price, BUP_CLZ_ID, BUP_CRG_ID, PricePerPackage, BUL_BUR_ID, CSL_Level StorageRedundancyLevel, CSL_Name StorageRedundancyLevelName, ItemLevelName
   ...
   , UniqueLoadBlock as
    (select distinct HST_CLV_ID, CBL_LBL_ID, PSH_Storage_BUL_ID, CBL_BufferedDiskSizeMB, cast(iif(HST_UseMonthlyIOPS = 1, CBL_AvgMonthlyIOPS/100000., CBL_BufferedIOPS) as bigint) IOPS, CBL_AvgMonthlyNetworkOutboundMB,
   ...
   , Summary as
    (select HST_CLV_ID, CRG_ID, CRG_CLZ_ID, CRG_Name, PSH_Storage_BUL_ID,
     ...
     group by HST_CLV_ID, CRG_ID, CRG_CLZ_ID, CRG_Name, PSH_Storage_BUL_ID with rollup
    )
   , SummaryUP as
    (select HST_CLV_ID, CRG_ID, CRG_CLZ_ID, CRG_Name, BillableItemType, Value, PSH_Storage_BUL_ID
     from Summary
   ...
   , BilledItems as
    (select CLV_ID,
     ...
     from SummaryUP
      ...
      inner join Pricing p on ItemType = BillableItemType  and HST_CLV_ID = p.CLV_ID
   ...
   , ItemPricingOptions as
    (select CLV_ID, ItemType, ItemLevelName, Units, PricePerUnit, Units * PricePerUnit AmountToPay, BUL_BUR_ID, StorageRedundancyLevel, StorageRedundancyLevelName
     from BilledItems
     where PricePerPackage is null
     union all
     select CLV_ID, ItemType, ItemLevelName, TotalUnits Units, PricePerPackage/TotalUnits PricePerUnit, PricePerPackage AmountToPay, BUL_BUR_ID, StorageRedundancyLevel, StorageRedundancyLevelName
     from BilledItems
     where PricePerPackage is not null
      and ValueRank = 1
    )
   , ItemPricingOptionsWithTotal as
    (select CLV_ID, ItemType, ItemLevelName, Units, PricePerUnit, AmountToPay, BUL_BUR_ID, StorageRedundancyLevel, StorageRedundancyLevelName,
   ...
   , ItemPricingOptionsRanked as
    (select CLV_ID, ItemType, ItemLevelName, Units, PricePerUnit, AmountToPay, BUL_BUR_ID, StorageRedundancyLevel, StorageRedundancyLevelName, TotalAmountPerItemLevel,
      dense_rank() over(partition by ItemType order by CLV_ID,TotalAmountPerItemLevel) PriceRank
     from ItemPricingOptionsWithTotal
    )
  select CLV_ID, ItemType, ItemLevelName, StorageRedundancyLevel, StorageRedundancyLevelName, sum(Units) Units,
  ...
  where PriceRank = 1
   or BUL_BUR_ID = 1
  group by CLV_ID, ItemType, ItemLevelName, StorageRedundancyLevel, StorageRedundancyLevelName
 )
GO
