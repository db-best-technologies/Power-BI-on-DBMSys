...
GO
/****** Object:  StoredProcedure [RuleChecks].[usp_TableWithVardecimalStorageEnabled]    Script Date: 6/8/2020 1:14:33 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create procedure [RuleChecks].[usp_TableWithVardecimalStorageEnabled]
	@ClientID int,
	@PRR_ID int,
	@FromDate date,
	@ToDate date,
...
select @ClientID, @PRR_ID, PTP_MOB_ID, IDB_ID, IDB_Name, DSN_Name, DON_Name
from Inventory.PossibleTableProblems
	...
	inner join Inventory.MonitoredObjects on MOB_ID = PTP_MOB_ID
	inner join Inventory.Versions on VER_ID = MOB_VER_ID
where PTP_IsVarDecimalStorage = 1
	and VER_Number >= 10
GO
