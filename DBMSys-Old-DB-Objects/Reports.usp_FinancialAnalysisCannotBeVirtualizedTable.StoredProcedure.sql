...
GO
/****** Object:  StoredProcedure [Reports].[usp_FinancialAnalysisCannotBeVirtualizedTable]    Script Date: 6/8/2020 1:14:33 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [Reports].[usp_FinancialAnalysisCannotBeVirtualizedTable]
as
set nocount on
...
select isnull(replace(replace(SQLEdition, '[', ''), ']', ''), '[Total]') [SQL edition],
	...
	iif(PricePerCore > 0, format(sum(LicensesOwned - VirtualizationUsedLicenses), '##,##0'), '') [Licenses owned],
	iif(PricePerCore > 0, format(sum(CoreCount - (LicensesOwned - VirtualizationUsedLicenses)), '##,##0'), '') [Licenses to be purchased],
...
group by SQLEdition, PricePerCore, LicensesOwned, VirtualizationUsedLicenses with rollup
having grouping(SQLEdition) = grouping(PricePerCore)
	and grouping(SQLEdition) = grouping(VirtualizationUsedLicenses)
