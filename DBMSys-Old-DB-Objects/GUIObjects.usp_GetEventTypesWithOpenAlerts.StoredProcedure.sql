...
GO
/****** Object:  StoredProcedure [GUIObjects].[usp_GetEventTypesWithOpenAlerts]    Script Date: 6/8/2020 1:14:33 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create procedure [GUIObjects].[usp_GetEventTypesWithOpenAlerts]
...
where exists (select *
				...
				where TRE_IsClosed = 0
					and TRE_MOV_ID = MOV_ID)'
	+ case when @SearchString is not null
			then + char(13)+char(10)
				+ 'and (MOV_Description like ''%'' + @SearchString + ''%'')'
			else ''
		end + '
