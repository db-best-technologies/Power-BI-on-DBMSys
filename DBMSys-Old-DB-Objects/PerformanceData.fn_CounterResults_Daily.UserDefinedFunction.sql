...
GO
/****** Object:  UserDefinedFunction [PerformanceData].[fn_CounterResults_Daily]    Script Date: 6/8/2020 1:14:32 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE function [PerformanceData].[fn_CounterResults_Daily](@StartDate datetime2(3),
...
return select MOB_Name ServerName, CSY_Name SystemName, CounterName, CIN_Name InstanceName, IDB_Name DatabaseName, CRS_DateTime SnapshotDate,
		...
		from (select CRS_MOB_ID, CRS_SystemID, CRS_CounterID, CRS_InstanceID, CRS_IDB_ID, CRS_DateTime, CRS_MinValue, CRS_AvgValue, CRS_MaxValue, CRS_SumValue
				...
				select CRS_MOB_ID, CRS_SystemID, CRS_CounterID, CRS_InstanceID, CRS_IDB_ID,
						CAST(CRS_DateTime as date) CRS_DateTime, min(CRS_MinValue), avg(CRS_AvgValue), max(CRS_MaxValue), sum(CRS_SumValue)
				from PerformanceData.CounterResults_Hourly
				where CRS_DateTime
						>= isnull((select dateadd(day, 1, MAX(CRS_DateTime)) from PerformanceData.CounterResults_Daily), '20000101')
